...
local function notify(msg, value)
  vim.notify(('%s %s'):format(msg, value and 'on' or 'off'), vim.log.levels.INFO)
end
local toggles = {
...
local function picker(name, cfg)
  return function()
    require('snacks.picker')[name](cfg or {})
  end
end
...
local mappings = {
  n = {
    ...
    ['<Leader>lf'] = {
      function()
        vim.lsp.buf.format({ timeout_ms = formatting.timeout_ms })
      end,
      desc = 'Format buffer',
...
local function check_cond(cond, client, bufnr)
  local t = type(cond)
  if t == 'function' then
    return cond(client, bufnr)
  end
  if t == 'string' then
    return client:supports_method(cond, bufnr)
  end
  if t == 'boolean' then
    return cond
  end
  ...
  callback = function(args)
    ...
    vim.lsp.buf.format({
      ...
      filter = function(client)
        return formatting_enabled(client)
      end,
    })
  end,
})
...
if features.codelens and vim.lsp.codelens.enable then
