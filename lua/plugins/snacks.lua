...
local function not_large(bufnr)
  if not (vim.api.nvim_buf_is_valid(bufnr) and vim.bo[bufnr].buftype == '') then
    return false
  end
...
local function picker(name, cfg)
  return function()
    require('snacks.picker')[name](cfg or { focus = 'input' })
  end
end
...
return {
  ...
  keys = {
    ...
    },
    {
      'gw',
      function()
        require('snacks.picker').projects({ confirm = add_workspace_folder })
      end,
